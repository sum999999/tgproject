<template>
  <aside v-if="isShowRightAside" class="layout-right-aside">
    <div class="inner">
      <ul>
        <li v-if="showLongqueue" class="redEnvelope" @click="onLongqueueClick">
        ...
        <!-- 聊天室 -->
        <li class="download" @click="openChatRoom">
          <div class="bg"><i></i></div>
          <!-- <span>{{ $tex("聊天室") }}</span> -->
        </li>
        ...
        <!-- <div class="bg"><img
                :src="$configText.feedbackImg ? '/configstatic/pc/images/feedback.png' : require('@/assets/images-v3/index/yjfk.png')"
                alt=""
              ></div><span>{{$configText.feedbackText ? $tex($configText.feedbackText) : $tex('意见反馈')}}</span> -->
        <!-- <div class="bg">
            <img :src="require('@/assets/images-v3/index/yjfk.png')" alt="" />
          </div>
          <span>{{ $tex("意见反馈") }}</span>
        </li> -->
        <!-- QQ客服 -->
...
export default {
  ...
  methods: {
    ...
    openService() {
      if (this.serverUrl) {
        Util.openWin(this.serverUrl.url);
      }
    },
    openHelpCenter() {
...
.layout-right-aside {
  ...
  ul {
    ...
    li {
      ...
      &.Customer {
        background: url("/src/assets/images/4.png");
        background-size: 85%;
        background-position: 5px;
        i {
          //   background-image: url("~@/assets/images/longqueue/yl_changlong.png");
