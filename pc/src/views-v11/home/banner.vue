<template>
  <div class="banner"
       @mouseenter="slideArrow = true"
       @mouseleave="slideArrow = false">
    <swipe class="my-swipe"
      ...
      <swipe-item v-for="(item, idx) in list"
                  :key="idx">
        ...
        <span :data="item.targetUrl"
              @click="goPage(item.targetUrl , item.resourceId)"><img :src="item.netUrl"></span>
      </swipe-item>
    </swipe>
    <span class="slide-to change-slide"
          :class="{'show-arrow': slideArrow}"
          @click="prev()">
      <i class="i-icon i-arrow-big-left"></i>
    </span>
    <span class="slide-next change-slide"
          :class="{'show-arrow': slideArrow}"
          @click="next()">
