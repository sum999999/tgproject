<template>
  <div class="redpacket-wrap">
    ...
    >
      <img
        class="hd"
        :src="currentMsg.content.user && currentMsg.content.user.portrait || defHdImg"
        alt=""
      >
      <p class="name">{{currentMsg.content.user && currentMsg.content.user.name}}</p>
      <p
        class="dec"
        v-if="currentMsg.content.extra.redpacketStatus=='00'"
      >{{currentMsg.content.extra.remark}}</p>
      <p
        class="dec"
        v-else
      >{{currentMsg.content.extra.redpacketDesc}}</p>
      ...
      <div
        class="look"
        v-if="currentMsg.content.extra.self "
    ...
    >
      ...
      >
        <img
          class="me-hd"
          :src="currentMsg.content.user.portrait  || defHdImg"
          alt=""
        >
        <p class="me-name">{{currentMsg.content.user.name}}</p>
        <p class="me-money">
          <span
            class="num"
      ...
      <div
        v-else-if="currentMsg.content.user"
        class="redpacket-info"
      >
        <img
          class="me-hd"
      ...
      <div
        class="redpacket-box"
        v-if="redpacketListInfo.redpacketList && currentMsg.content.user.id == user.userId"
      >
  ...
  data() {
    ...
    const validateNum = (rule, value, callback) => {
      ...
      if (!/^[0-9](\d){0,1000000}$/.test(value)) {
        ...
        callback(this.$tex("红包个数必须小于等于{num}", {num: redpacketChatConfig.redpacketMax}));
      } else {
        callback();
      }
    };
    const validaPayPwd = (rule, value, callback) => {
      if (!/^(\d){3,6}$/.test(value)) {
  ...
  methods: {
    ...
    clkRedpacketList() {
      ...
      this.$http
        .post(
          ...
          {
            redpacketChatId: this.currentMsg.content.extra.redpacketChatId
          },
    ...
    getRedpacket(item, roomNo) {
      ...
      if (
        ...
        item.state == 0 ||
        !this.currentMsg.content.extra.redpacketChatId
      )
        return;

      if (this.redpacketStatusLoading) {
        return;
      }
      this.redpacketStatusLoading = true;
      this.$http
        .post(
        ...
        .then(response => {
          this.redpacketStatusLoading = false;
          this.openLoad = false;
          if (response.data.code !== 0) return;
          this.currentMsg.content.extra.self = response.data.data.self;
          this.currentMsg.content.extra.redpacketStatus = response.data.data.redpacketStatus; //00:可领,01:过期,02:已领,03:领完
          this.currentMsg.content.extra.redpacketDesc = response.data.data.redpacketDesc;
          if (this.currentMsg.content.extra.redpacketStatus == "02") {
          ...
          if (vm.user.userCode) {
            ...
            this.$http
              ...
              .then(response => {
                ...
                const message = new RongYun.MessageManager.RedPacketMessage({
                  content: '',
                  extra: {
                    type: "103",
      ...
      this.$http
        ...
        .then(response => {
          ...
          if (0 == code) {
            ...
            this.currentMsg.content.extra.redpacketStatus = "03";
            this.currentMsg.content.extra.redpacketDesc = response.data.msg;
          } else {
