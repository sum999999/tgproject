<template>
  <div class="sebo">
    ...
    <div
      :class="{
        sansebo: tabId === '701301',
        banbo: tabId === '701302',
        banbanbo: tabId === '701303',
        qisebo: tabId === '701304',
      }"
    >
        ...
        <ul>
          ...
          >
            <div class="container">
              ...
              <span class="num" v-show="tabId !== '701304'">{{
                $tex("号码")
              }}</span>
            </div>
          </li>
          ...
          >
            <div class="container">
              ...
              <span class="num" v-if="item.num && item.num.data">
                <i
                  class="ball"
                  v-for="value in item.num.data"
                  :key="value"
                  :style="{ background: item.num.clr }"
                >
                  {{ value }}
                </i>
              </span>
...
export default {
  ...
  methods: {
    ...
    changeList(item) {
      ...
      this.playTipInfo.playDesc = util.escapeHtml(this.playTipInfo.playDesc);
    ...
    this.tabList.forEach((item) => {
      if (item.id === "701302" || item.id === "701303") {
        item.children[0].layout.rates.forEach((value) => {
          if (item.id === "701302") {
            value.num = liuhe.banbo.filter(
              (vvv) => vvv.label === value.ball
            )[0];
          } else {
            value.num = liuhe.banbanbo.filter(
              (vvv) => vvv.label === value.ball
            )[0];
          }
        });
      } else if (item.id === "701301") {
        //手动添加三色波的号码
