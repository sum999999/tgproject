  ...
  <div>
    <div class="field-item">
      ...
      <div class="field-item__content">
        <span class="convert-blance">{{$tex("￥")}}{{userBalance}}</span>
      </div>
    </div>
    <div class="field-item">
      <span class="field-item__title">转入余额：</span>
      <div class="field-item__content">
    ...
    <div class="field-item">
      ...
      <div class="field-item__content">
        <Button type="primary"
                ...
                @click="onTransferClick">确认转入</Button>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    userBalance () {
      return numeral(this.balance).format('0,0.00', Math.floor) || '0.00'
    },
    disTransfer () {
      if (
        !!this.transInVal &&
  ...
  methods: {
    ...
    onTransferClick () {
      this.createTransfer({
        ...
        this.$store.commit('getUserBalance', this.userBalance - this.transInVal)
        this.$Message.success('转入成功');
        this.$emit('tab-replace-pageview', 'YebTransferResult', {
          title: '转入成功',
          dsc: `成功转入${this.transInVal}元至余额宝账户余额`
        })
      })
    }
  }
