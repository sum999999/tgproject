<template>
  <div class="bankAccount">
    <template v-if="hadBind">
      <accountCard>
        <img src="/static/images/txdwx02.png" alt="" slot="cardImg" />
    ...
    <template v-else>
      <div class="bankAccount-content">
        <Form
          ...
          :rules="addCardRule"
        >
          <FormItem :label="`${$tex('微信号')}:`" prop="bankNo">
            <Input
              type="text"
...
export default {
  ...
  data() {
    ...
    const accountCheck = (rule, value, callback) => {
      if (!value) {
        callback(new Error(this.$tex("请输入微信账号")));
      }
      if (
        !/^[a-zA-Z]([-_a-zA-Z0-9]{5,19})+$/.test(value) &&
        !/^1(\d){10}$/.test(value) &&
        !/^[1-9]\d{5,11}$/.test(value)
      ) {
        callback(new Error(this.$tex("账号为：微信号，QQ号或者手机号")));
      }
    };
    ...
    return {
      ...
      addCardRule: {
        ...
        bankNo: [
          { message: this.$tex("微信号不能为空"), trigger: "blur" },
  ...
  methods: {
    getBankList() {
      ...
      vm.$http
        ...
        .then((response) => {
          ...
          vm.list = data.lotUserWechatList;
          if (vm.list.length) {
            vm.hadBind = true;
          }
        });
    ...
    toAddBank() {
      ...
      this.$emit("weChatRealNameReg", true);
    },
    cancel() {
      this.bandBankFlag = false;
    ...
    addFile(event) {
      ...
      var newsrc = vm.getObjectURL(_files[0]);
...
.uploadFiles {
  ...
  margin: auto;
  position: relative;
  background: url("/static/images/wx.png") center center no-repeat;
...
.icon-zfb {
  width: 25px;
  height: 25px;
  background: url(/static/images/weixin.png) center center no-repeat;
  background-size: contain;
}
</style>
