<template>
  ...
  <div class="lottery-list">
    <div class="list">
      <div class="">
        <h5>
          <i class="i-icon i-lottery"></i>{{$tex("购彩大厅")}}
        </h5>
        <ul>
          <li
          ...
          >
            ...
            <div class="child rq-nice-scroll">
              <p
    ...
    <div class="streamer">
      ...
      <div class="rt">
        ...
        >
          ...
          <span class="record">{{$tex("游戏玩法")}}</span>
        </Button>
        <Button
          type="primary"
        ...
        >
          ...
          <span class="record">{{$tex("开奖公告")}}</span>
        </Button>
        <Button
          type="primary"
        ...
        >
          ...
          <span class="record">{{$tex("走势图表")}}</span>
        </Button>
        <Button
          type="primary"
          @click="goRecord"
        >
          ...
          <span class="record">{{$tex("投注记录")}}</span>
        </Button>
      </div>
    </div>
...
import announce from '../../common/module_vue/annouce'
...
export default {
  ...
  created() {
    ...
    queryLotteryList().then((response) => {
      ...
      this.$nextTick(() => {
        let query = this.$route.query.fromHomeCode
        if (query) {
          // this.lotteryType = query;
          let lotteryItemList = this.lotteryTypeList.filter((item) => item.lotteryType === query)[0]
          // console.log(lotteryItemList);
          this.setlotteryType({
            lotteryType: query,
            lotteryList: lotteryItemList.lotteryList.length,
          })
        }
      })
    })
