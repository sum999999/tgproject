...
export default {
  ...
  methods: {
    goUrl(event) {
      ...
      let banner = null;
      if (
        event.srcElement.slideManager &&
        event.srcElement.slideManager.newSlide
      ) {
        imgUrl = event.srcElement.slideManager.newSlide.src;
        banner = this.list.find((item) => item.netUrl == imgUrl);
      } else {
        banner = this.list[0];
      }
...
.banner-wrap @{deep} {
  ...
  .cs-circleslight .br-large-loading .img {
    background: url("~@/assets/images/cute3d/loading.gif") no-repeat center
      center;
  }
}
</style>
