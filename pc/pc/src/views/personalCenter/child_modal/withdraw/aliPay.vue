...
<template>
  <div class="withdrawAll">
    <div v-if="stepOne && hadPayPwd">
      ...
      <!-- :modalShow="bandBankFlag && showBandModal" -->
      <modal
        :modalShow="bandBankFlag"
        :title="
          lotUserBankList.length > 0 ? $tex('绑定银行卡') : $tex('绑定支付宝')
        "
        @btn-ok="toAddBank"
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      let data = this.data;
      this.lotUserBankList = data.lotUserAlipayList;
